...
`plan-tf` and `apply-tf` stay separate jobs with an artifact handoff between them, deliberately — not collapsed into one job. That split is what makes the saved plan reviewable and lets the Environment's required-reviewer rule sit between "here's what would change" and "make it happen." Collapsing them would trade that away for less job-graph complexity — not worth it for a control plane.
...
| `AWS_STATE_ROLE_ARN` | `tf-control`, per-Environment secret | `plan-tf.yml`, `apply-tf.yml`, `test-endpoint.yml` (fallback path) |
| `AWS_DEPLOY_ROLE_ARN` | `tf-control`, per-Environment secret | `plan-tf.yml`, `apply-tf.yml` |
...
GitHub Environment names must match the `environment:` input passed to `plan-tf.yml`/`apply-tf.yml` exactly (`sandbox`, `dev`, `test`, `prod`, `management`) — the IAM role trust policy's `sub` claim is scoped to `repo:helix-infrastructure/tf-control:environment:<name>`, and the Environment's required-reviewer protection rule is what actually gates `apply-tf.yml` (no custom approval logic in the workflow itself).
